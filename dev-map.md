...
## 共享比较管线（fastcheck-compare-pipeline，抽取 FastClone/FastCheck 公共 compare worker 编排）

`docs/tasks/fastcheck-compare-pipeline/`。FastClone 客户端与 FastCheck 各自内联了一套「compare worker 池 + 任务/结果队列 + `DecideCompare` 调用 + 背压」编排，逻辑重复。本任务把这套编排抽成**无状态**共享组件 `fc::ComparePipeline`，两端接入之；比较判定唯一真源仍是 `compare_phase.h` 的 `DecideCompare`（组件内 worker 复用、不内联复制，红线不改其/`ClassifyByHash`/mtime 语义）。R-01 采**候选 A**（decisions.md D-01）：strict 探测前移后**仅 size 相等文件发 `HashRequest`**，Missing/SizeDiff 在 compare drain 路径直接 `record`，移除 eager 每文件发 + `finalizePaired` 记账回退。

| 落点 | 文件 / 符号 | 做法 |
|---|---|---|
| 组件本体 | `FastClone/compare_pipeline.h/.cpp` `fc::ComparePipeline` | 固定 `workerCount` 线程池；`ComparePipelineConfig{mode, workerCount, batchPop}` + 注入 `LocalProbeFn probe`（`rel→optional<FileEntry>`，probe 抛异常吞为 `nullopt`→Missing）+ `onResultsReady` 回调。**单生产者** `Enqueue`（写 `dispatchBuffer_`，`issued_` 即时自增以让 `InFlight()` 门控准确）/`Flush`（一次性推入 `tasks_`+`notify_all`）；worker 批量 `batchPop` 取任务→`probe`→`DecideCompare(mode,…)`→`results_`；`Drain(out)` swap 出 `ComparedItem{remote, local, outcome}` 并 `drained_`/`pending_` 结算。`InFlight()=issued-drained`、`QueuedTasks`、`PendingResults`/`HasResults`、`Stop`/`Join`（析构兜底 `Stop+Join`）。 |
| FastClone 接入 | `FastClone/sync_engine_client.cpp` `RunClient` | 删原内联 `CompareTask`/`CompareResult`/队列/互斥/CV/worker lambda/atomics，换 `fc::ComparePipeline comparePipeline`（mode=Fast，`onResultsReady` 唤醒 `compareResultCv`）。`enqueueCompareTask/flushCompareDispatch` 转发 `Enqueue/Flush`；主循环 `comparePipeline.Drain(ready)`→`CompareActionFromOutcome(item.outcome)`→**不变**的 `handleCompareResult(relPath, action)` 状态机；背压门 `InFlight()>=maxInFlightCompareTasks(=max(8192,worker*256))`→`delayedCompareEntries` 溢出缓冲；诊断行改用 `InFlight/QueuedTasks/PendingResults`；收尾 `Stop()`+`Join()`（正常+catch 双路径）先于 `clientDriver` 析构（AC-16）。 |
| FastCheck 接入 | `FastCheck/check_engine.cpp` `RunCheck` | recv 主线程对**任何** mode 均**不** probe（取代 S-01 的 strict 专用分叉与 Fast/SizeOnly recv 内联 `ProbeLocal`）：仅 `manifestPaths.insert` + `comparePipeline.Enqueue`（溢出 `delayedCompareEntries`）。`probe` = Fast/SizeOnly→`ProbeLocal`（size+mtime）/ Strict→`StrictProbe`（size-only `directory_entry`，逐字保留 RS-01 目录判定）。`drainCompare`：`outcome.needHash`→`hashQueue.push_back(HashNeed{remote, local})`+记账（候选 A：**仅 size 相等**），否则 `record(category, local)`（Missing/SizeDiff 直接定案，无记账回退）。`finalizePaired` 仅剩 `Hashed`/`Failed`（`LocalResult::Kind` 删 `Missing/SizeDiff`；`HashTask` 删 `deferred`；worker 只 hash）。收尾 `comparePipeline.Stop/Join` 先于 `joinWorkers()` 先于 `clientDriver` 析构。 |
| 测试 | `tests/test_compare_pipeline.cpp` `RunComparePipelineTests` | 批量 enqueue/flush/drain、并行 drain、stop/join、probe 抛异常→Missing、bounded in-flight、计数一致性（`issued/drained/pending`）。登记 `tests/test_main.cpp` + `CMakeLists.txt` `FastCloneTests`。 |
| 构建（4 target） | `CMakeLists.txt` | `FastClone`、`FastCloneTests`、`FASTCHECK_SHARED_SOURCES`（供 `FastCheck`/`FastCheckTests`）均追加 `FastClone/compare_pipeline.cpp`；4 个 `.vcxproj` 同步登记 `compare_pipeline.h/.cpp`（+`FastCloneTests.vcxproj` 加 `tests/test_compare_pipeline.cpp`）。 |

### 惯用法 / 约束（CP-01..CP-05）
- **CP-01 单一真源**：比较判定只经组件内 `DecideCompare(cfg_.mode,…)`；`ClassifyByHash`（hash 定案）仍在两端主线程 `finalizePaired`/`handleCompareResult`。组件**只**做 probe+DecideCompare+结果搬运，**不**碰传输/报告/hash 调度/记账。
- **CP-02 单生产者 Enqueue/Flush**：`Enqueue` 仅可由单一主/recv 线程调用（`dispatchBuffer_` 非锁保护）；`issued_` 在 `Enqueue` 即时自增（缓冲未 Flush 也计入 `InFlight()`，背压门与循环存活条件准确）；`Flush` 每轮循环调用一次，缓冲项至多滞后一轮才被 worker 可见。
- **CP-03 背压**：`InFlight()>=cap` 时**停止**从 manifest/网络拉新条目，溢出停 `delayedCompareEntries`，`Drain` 释放 in-flight 后再回填（`InFlight()<cap`）。cap 两端一致 `max(8192, workerCount*256)`。唤醒模型不变：无帧可收且仅本地 compare/hash 待完成 → `wait_for(200ms, HasResults()||…)` 防死锁。
- **CP-04 候选 A（strict/Fast 统一）**：`needHash==true`（Fast mtime-miss / Strict size-equal）才入 `hashQueue`+发 `HashRequest`；Missing/SizeDiff 在 drain 直接 `record`，**不**发 HashRequest、**不**记账回退。取代 strict-defer S-02/S-04/S-05 与 redundant-syscall RS-01 的 worker `deferredStat` 定案（那套 worker 内 `directory_entry`+`Missing/SizeDiff` 回退整体删除；strict 目录/大小判定语义由 `StrictProbe` 在 compare worker 承接，逐字等价）。用户可见语义（分类/报告/退出码/hash 字节）不变。
- **CP-05 生命周期（AC-16）**：`comparePipeline` 声明于 `clientDriver` 之后 → 析构早于 `clientDriver`；且正常与 catch 两条收尾路径都显式 `Stop()`+`Join()`（FastCheck 再 `joinWorkers()`）后才让 `clientDriver` 出作用域，保证 worker 引用的 driver/CV 存活期覆盖。

### 测试 / 构建（compare-pipeline）
- **MockChannel 请求响应模式（`FastCheck/tests/test_check_engine.cpp`）**：并行 compare 后 `HashRequest` 发送顺序不再等于 manifest 顺序；`MockChannel` 原按固定顺序预置 `HashResponse`，window=1（`--checkers 1`）时客户端先请求的文件与首个预置响应错位 → 响应被丢弃 → EOF。新增 `useResponder`+`hashResponder` 映射：`send` 收到 `HashRequest` 即按 rel 追加对应 `HashResponse` 到 `inbound`，**忠实建模真实服务端**（逐请求响应，线上响应顺序恒等于请求顺序）。仅 `TestCheckersOneSerializesHash` 改用之；window≥文件数的用例天然顺序无关，不受影响。
- **strict 用例更新（候选 A）**：`TestStrictMissingViaWorker`/`TestStrictSizeDiffViaWorker` 断言 `HashRequest==0`（原 1）；`TestStrictOrderingMissingDiff` `==1`（原 3，仅 size-equal）；`TestStrictMixed` `==2`（原 4）。分类计数（same/diff/missing/extra）全部不变。
