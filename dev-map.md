...
| 文件 | 职责 | 关键符号 | 平台守卫 |
|---|---|---|---|
| `FastClone/disk_io_align.h/.cpp` | 运行时对齐查询 + 平台对齐分配（零硬编码 4096/512） | `AlignInfo`、`QueryAlign`、`QueryPageSize`、`AlignUp/Down`、`IsAligned`、`AlignedAlloc/Free`、`kSmallFileBufferedMax` | Win `GetDiskFreeSpaceW`+`_aligned_malloc`；Linux `BLKSSZGET`/`statvfs`+`posix_memalign`；mac `statvfs`+`posix_memalign` |
...
## 测试落点
- `tests/test_disk_io_align.cpp` → `RunDiskIoAlignTests`（对齐数学/16K/64K 替身/对齐分配匹配；F5：`TestQueryAlignAscii` ASCII 回归、`#if _WIN32` `TestQueryAlignNonAsciiWindows` 非 ASCII UTF-8 路径）
