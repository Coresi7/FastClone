...
- Scales to tens-of-millions-file, TB-scale trees: tiny files are batched together while many files run concurrently across multiple streams / links (a single file itself is never split across streams; the opt-in `--large-file-block` block mode is the deliberate exception, see below)
...
- `--large-file-block [<size>]`: **opt-in, default OFF**. Supplying it (with or without `<size>`) enables large-file block mode: files `>= --large-file-threshold` are sliced into blocks fetched in parallel across **all** healthy links, then verified whole-file (XXH3-128) before an atomic rename. `<size>` is optional — when omitted the 32 MiB reference block size is used; with a value, a `K|M|G` suffix is accepted (range `1M..4G`, power of two; default `32M`). Takes effect only with `>= 2` healthy links and a peer advertising the file-range capability (FC7, both ends on this version); otherwise it falls back to the legacy single-stream path with unchanged behavior. Small files and single-link syncs are never affected. When block mode is on, `--large-file-lane` applies only to large files NOT in block mode. Independent of `--large-file-threshold` (the threshold decides WHAT is large; the block size only decides HOW BIG each slice is)
...
- **No block-level resume**: an interrupted large file is re-sent from the start on the legacy whole-file path (no offset field). Exception: with `--large-file-block` on, a single link drop mid-transfer re-routes only that link's in-flight blocks (re-sent whole; completed blocks are not re-fetched) — block-level resume within the session
...
- No block-level resume by default; rely on auto-reconnect or rerun after interruption (unchanged files are still skipped by comparison). With `--large-file-block` on, a link-level interruption resumes at block granularity within the session (in-flight blocks are re-routed and re-sent whole, completed blocks are not re-fetched); cross-process / cross-session block resume is still unsupported
