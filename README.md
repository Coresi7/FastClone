...
- Multi-connection parallel transfer (multipath / FC7): aggregate bandwidth across multiple NICs; also works fine with a single NIC / connection
...
- Scales to tens-of-millions-file, TB-scale trees: tiny files are batched together while many files run concurrently across multiple streams / links (a single file itself is never split across streams; the `--large-file-block` block mode is the deliberate exception — auto-enabled by default on multi-link setups, see below)
...
- `--large-file-lane`: how large files (`>= --large-file-threshold`) are routed across links **on the legacy single-stream path**: `primary` (pin to the primary link), `aux` (schedule by weight like any other file), or `auto` (prefer aux when `--aux-weight >= 2.0`, otherwise pin to primary); default `auto`. **Mutually exclusive with block mode**: giving `--large-file-lane` at all (any value, including `auto`) opts out of block-mode auto-activation (auto is treated as off); combining it with `--large-file-block <size>` (forced on) is rejected as an error naming both flags; only `--large-file-block off` + `--large-file-lane` is a legal combination
- `--large-file-block [auto|off|<size>]`: three-state control of large-file block mode; **the default is auto** (flag absent, given without a value, or the literal value `auto` are equivalent). In auto, block mode activates whenever the peer advertises the file-range capability (FC7, both ends on this version), `>= 2` healthy links are up, and `--large-file-lane` is not given: files `>= --large-file-threshold` are sliced into blocks (the 32 MiB reference size) fetched in parallel across **all** healthy links, then verified whole-file (XXH3-128) before an atomic rename — **multi-link setups now enter block mode by default** (a default-behavior change); if any condition fails it falls back to the legacy single-stream path automatically. An explicit `<size>` (`K|M|G` suffix, range `1M..4G`, power of two) is the only force-on entry and uses that block size, still subject to the peer capability and the healthy-link count (`--large-file-block 32M` = force-on with the default block). `off` is the explicit escape hatch: block mode never activates, and transfer paths, on-disk bytes, stream-slot allocation and log frame types are byte-for-byte identical to versions before block mode existed (the zero-regression promise is anchored to this off form, no longer to the default). `on` is not a legal value (rejected as an illegal value). Small files and single-link syncs are never affected. Independent of `--large-file-threshold` (the threshold decides WHAT is large; the block size only decides HOW BIG each slice is). Mutually exclusive with `--large-file-lane` (see that entry)
...
## Multi-NIC Parallel Transfer (Multipath, FC7)
...
- **File-to-link scheduling**: by default a full-file transfer is not split across links. Regular files and small-file batches are distributed across healthy links by a weighted shortest-queue policy (the link with the fewest outstanding streams wins; `--aux-weight` biases the choice toward aux links). Large files (`>= --large-file-threshold`, default `1G`) follow `--large-file-lane`: pinned to the primary link by default (assumed best), or scheduled by weight like other files when aux is preferred. (Two exceptions — a delta'd file's changed ranges *are* spread across links (see below; delta is opt-in); and when block mode is active — auto-activated by default on multi-link setups with this server version, or forced on with an explicit `<size>` — a large file is fetched as blocks across ALL healthy links, with a mid-transfer link drop re-routing only its in-flight blocks, re-sent whole. Note `--large-file-lane` is mutually exclusive with block mode: giving it opts out of auto/forced activation (auto folds to off; `<size>` + lane is an error), and only `--large-file-block off` + lane is legal.)
...
- **No block-level resume**: an interrupted large file is re-sent from the start on the legacy whole-file path (no offset field). Exception: when block mode is active (auto-activated by default with multiple healthy links + this server version, or forced on with an explicit `--large-file-block <size>`), a single link drop mid-transfer re-routes only that link's in-flight blocks (re-sent whole; completed blocks are not re-fetched) — block-level resume within the session
- The current wire protocol is **FC7** (multipath session + capability bits: adds `SessionJoin`, and `AuthOk` carries the session id, the server endpoint list and the capability byte); with multiple connections, a single dropped link migrates its in-flight files to healthy links, and only a fully-down pool triggers a session-level reconnect
...
- No block-level resume by default; rely on auto-reconnect or rerun after interruption (unchanged files are still skipped by comparison). When block mode is active (auto by default on multi-link setups with this server version, or forced on via `--large-file-block <size>`), a link-level interruption resumes at block granularity within the session (in-flight blocks are re-routed and re-sent whole, completed blocks are not re-fetched); cross-process / cross-session block resume is still unsupported
