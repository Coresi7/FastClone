...
// Multipath network topology helpers (design §5.2, §6.1, §6.2, §6.4). All logic here is
...
struct LocalAddress {
    ...
    int prefixLen = 0;        // on-link prefix length (0 = unknown -> default /24 or /64, §4.3)
};
...
// same-NIC dedup so auxiliaries cannot reuse it (FR-009 / design §6.4).
...
// A chosen client->server pairing for one connection (design §6.4).
struct LinkPlan {
...
// Address-family preference rank exposed for testing (L-r6-02, §4.3): IPv4 -> 0, IPv6 -> 1
...
// Same-subnet test exposed for testing (D-02 / §4.3). True when clientIp and serverHost are
...
// rewritten per r6-route-quality §4 to edge-scored greedy bipartite matching):
...
// so auxiliaries never reuse the primary's NIC on EITHER side (FR-009 / §4.4).
