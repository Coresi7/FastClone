...
- 面向千万量级文件、TB 级数据的超大目录树：碎文件批量打包传输，多个文件在多条流 / 多条链路上并发（单个文件本身不跨流拆分；`--large-file-block` 大文件块模式是刻意的例外——多链路环境下缺省自动启用，见下文）
- 协议版本强校验（版本不匹配直接拒绝）
...
- `--large-file-lane`：大文件（`>= --large-file-threshold`）在 **legacy 单流路径**上各链路间的路由方式：`primary`（固定走首要链路）、`aux`（与普通文件一样按权重调度）、`auto`（`--aux-weight >= 2.0` 时倾向辅助链路，否则固定走首要链路）；默认 `auto`。**与块模式互斥**：只要显式给出 `--large-file-lane`（任意值，含 `auto`）即视为放弃块模式的自动开启（auto 视为关闭）；与 `--large-file-block <size>`（强制开启）同用会被拒绝报错（错误信息含两个参数名）；仅 `--large-file-block off` + `--large-file-lane` 为合法共存
- `--large-file-block [auto|off|<size>]`：大文件块模式的三态控制，**缺省为 auto**（不传该参数、给无值形式、或显式 `auto` 三者等价）。auto 态下，当对端通告 file-range 能力（FC7 两端均为本版本）、健康链路数 `>= 2`、且未显式给出 `--large-file-lane` 时，块模式自动启用：`>= --large-file-threshold` 的文件被切块（参考块大小 32 MiB），跨**全部**健康链路并行拉取，落盘后对整文件做 XXH3-128 校验再原子改名——**多链路环境升级后默认即进入块模式**（默认行为变更）；任一条件不满足即自动回退原单流路径。显式 `<size>`（`K|M|G` 后缀，范围 `1M..4G`，必须为 2 的幂）为强制开启的唯一入口，块大小取该值，仍受对端能力位与健康链路数的客观约束（`--large-file-block 32M` 即"强开 + 默认块"）。`off` 为显式关闭逃生门：块模式恒不激活，传输路径、落盘字节、流槽分配与日志帧类型均与引入块模式前的旧版逐字节一致（zero-regression 承诺锚定于该关闭形式，不再挂缺省态）。`on` 不是合法取值（按非法值报错）。小文件与单链路同步不受影响。与 `--large-file-threshold` 相互独立（阈值决定"哪些文件算大"，块大小只决定"切多大"）。与 `--large-file-lane` 互斥（见该参数条目）
...
- **非块级断点续传**：大文件若传输中断，重连后从文件头重传（默认整文件路径无 offset 字段）。例外：块模式激活时（缺省 auto 在多链路 + 本版服务端下自动激活，或显式 `--large-file-block <size>` 强开），会话内单条链路断开只按块重路由重传在途块，已完成块不重传（同会话内块级续传）
...
- **文件到链路的调度**：整文件传输默认不跨链路拆分。普通文件与碎文件批按"加权最短队列"策略分摊到各健康链路（在途流最少的链路优先；`--aux-weight` 让选择向辅助链路倾斜）。大文件（`>= --large-file-threshold`，默认 `1G`）按 `--large-file-lane` 路由：默认固定走首要链路（假定其为最佳链路），或在倾向辅助链路时与普通文件一样按权重调度。（两个例外——delta 文件的变化范围**会**跨链路分摊（见下文，delta 为可选开启）；块模式激活时（缺省 auto 在多链路 + 本版服务端下自动激活，或显式 `<size>` 强开）大文件按块跨全部健康链路并行拉取，会话内链路断开仅在途块整块重路由重传。注意 `--large-file-lane` 与块模式互斥：显式给出 lane 即放弃块模式的自动/强制开启——auto 视为关闭、`<size>` 强开报错，仅 `--large-file-block off` + lane 合法共存。）
...
- 镜像模式会删除客户端多余文件/目录
- 默认不支持单文件块级断点续传；中断后依赖自动重连或重跑（重跑/重连均可自动对比，相同文件无需二次传输）。块模式激活时（缺省 auto 在多链路 + 本版服务端下自动激活，或显式 `--large-file-block <size>` 强开），会话内链路级中断按块续传（在途块整块重路由，已完成块不重传）；跨进程/跨会话的块级续传仍不支持
